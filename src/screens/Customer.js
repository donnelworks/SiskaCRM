...
const Customer = ({navigation}) => {
  ...
  function loadData() {
    axios.get(`${Url.api}/customer/data`).then((res) => {
  ...
  const tglJam = (str) => {
    ...
    return (
      tanggal + ' ' + bulanIndo[Math.abs(bulan)] + ' ' + tahun + ' - ' + time
    );
  };
  ...
  const hapusData = (id) => {
    ...
    axios
      .delete(`${Url.api}/customer/data/hapus/${id}`)
      .then((res) => {
        setLoading(false);
        loadData();
      })
  ...
  const tags = (tag) => {
    axios
      .get(Url.api + '/customer/data/load_tag', {
        params: {
          tag: tag,
        },
      })
    ...
    <SwipeRow disableLeftSwipe={true} leftOpenValue={160}>
      <View style={styles.opsiBack}>
        <TouchableOpacity
          ...
          onPress={() => editKonfirm(rowMap, data.item.id_customer)}>
          <Icon name="square-edit-outline" size={25} color={Warna.light} />
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.hapusWrapper}
          onPress={() => hapusKonfirm(rowMap, data.item.id_customer)}>
          <Icon name="trash-can-outline" size={25} color={Warna.light} />
        </TouchableOpacity>
      </View>

      <Ripple
        rippleColor={Warna.softDark}
        style={styles.customerList}
        onPress={() => showModal(data.item.id_customer)}>
        <View style={styles.avatarWrapper}>
          <View style={styles.avatar}>
            <Text style={GlobalStyle.textAvatar}>
              {avatar(data.item.nama_customer)}
            </Text>
          </View>
        </View>
        <View style={styles.centerWrapper}>
          ...
          <Text
            style={{
              ...
              fontSize: 12,
              color: Warna.softDark,
            }}>
            {data.item.tlp_customer}
          </Text>
        </View>
        <View style={styles.rightWrapper}>
          ...
          <Text
            ...
            {data.item.credit_customer == 1
              ? 'Cash'
              : data.item.credit_customer == 2
              ? 'Credit Ok'
              : 'Credit Lancar'}
          </Text>
        </View>
  ...
  const ContentModal = () => (
    <View style={styles.modalContentWrapper}>
      ...
      <View style={{minHeight: 50, justifyContent: 'center', marginBottom: 10}}>
        ...
        <Text style={styles.textDetail}>
          {alamatNpwpDetail == null ? '-' : alamatNpwpDetail}
        </Text>
      </View>
      ...
      <View style={{minHeight: 50, justifyContent: 'center', marginBottom: 10}}>
        ...
        <Text style={styles.textDetail}>
          {alamatKantorDetail == null ? '-' : alamatKantorDetail}
        </Text>
      </View>
      ...
      <View style={{minHeight: 50, justifyContent: 'center', marginBottom: 10}}>
        ...
        <Text style={styles.textDetail}>
          {alamat2Detail == null ? '-' : alamat2Detail}
        </Text>
      </View>
      ...
      <View style={{minHeight: 50, justifyContent: 'center', marginBottom: 10}}>
        ...
        <Text style={styles.textDetail}>
          {profileDetail == null ? '-' : profileDetail}
        </Text>
      </View>
        ...
        <View style={{flexDirection: 'row'}}>
          {tagDetail.map((data) => {
            return data.length != 0 ? (
              <TagsList key={data.id_tag} tag={data.nama_tag} />
            ) : (
      ...
      <View style={{minHeight: 50, flexDirection: 'row', marginBottom: 10}}>
        ...
        <View style={{flex: 1, minHeight: 50, justifyContent: 'center'}}>
          ...
          <Text style={styles.textDetail}>
            {ubahCustomerDetail == null ? '-' : tglJam(ubahCustomerDetail)}
          </Text>
        </View>
      </View>
    </View>
  ...
  const ModalKonfirm = ({id}) => (
    <Modal animationType="fade" transparent={true} visible={modalKonfirm}>
        ...
        <View style={styles.modalBody}>
          ...
          <View style={styles.btnKonfirmWrapper}>
            <View style={{flex: 1, paddingRight: 5}}>
              <Button type="primary" onPress={() => hapusData(id)}>
                Iya
              </Button>
            </View>
            <View style={{flex: 1, paddingLeft: 5}}>
              <Button type="disabled" onPress={() => setModalKonfirm(false)}>
                Tidak
              </Button>
            </View>
          </View>
